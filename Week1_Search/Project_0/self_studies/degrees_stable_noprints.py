...
###################################################
## On Average 3x Faster than WithPrints version. ##
###################################################
...
def main():
    ...
    source = person_id_for_name("Minnie Driver")    # Jennifer Ehle
    if source is None:
        sys.exit("Person not found.")
    # target = person_id_for_name(input("Name: "))
    target = person_id_for_name("Jennifer Ehle")  # Leonardo DiCaprio
    if target is None:
        sys.exit("Person not found.")
...
def shortest_path(source, target):
    ...
    while not frontier.empty():
        ...
        if node.state == target:
            ...
            path = []
            # Append until reaching source, for which we do not need to append Action
        ...
        for action, state in neighbors:
            if not frontier.contains_state(state) and state not in explored:
                ...
                frontier.add(child)
                # print(f"Added to frontier: {state} via action {action}")
            # else:
