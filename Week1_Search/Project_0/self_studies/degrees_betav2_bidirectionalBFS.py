...
# Works but because of shortest_path algorithm, does not always find shortest path.
# Let's define 1st level search the first node, 2nd level the second, - etc
# The problem is simple - sometimes backward path reaches a higher lvl search than forward.
# (or even equal)
# This means that we are no longer looking for the shortest path, but only for a path - do you understand?
# We need to somehow always keep the forward search level one index in front of the backward - or maybe you have some other ideas,
